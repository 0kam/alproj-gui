...
match, plot = image_match(
    ...
    method="superpoint-lightglue", plot_result=True, outlier_filter="fundamental", params=params_init, resize=800, threshold=30.0,
...
params_2nd, error = cma_optimizer.optimize(
    generation = 300, sigma = 1.0, population_size=150, f_scale=10.0)
print("Error:", error)
...
match, plot = image_match(
    ...
    method="superpoint-lightglue", plot_result=True, outlier_filter="essential", params=params_2nd, resize=800, threshold=30.0,
...
cma_optimizer.set_target(["fov", "a1", "k1", "k2", "k3", "p1", "p2", "s1", "s3"])
# cma_optimizer.set_target(["fov", "k1", "k2", "k3", "k4", "k5", "k6", "p1", "p2", "s1", "s2", "s3", "s4"])
params_optim, error = cma_optimizer.optimize(
    generation = 300, sigma = 0.1, population_size=150, f_scale=error*2)
print("Error:", error)
print("Optimized Parameters:")
for k in params_optim.keys():
    print(f"{k}: {params_optim[k]}")

print("---- Further optimization for k3, k4, k5, k6, s2, s4 ----")
cma_optimizer = CMAOptimizer(gcps[["x","y","z"]], gcps[["u","v"]], params_optim)
cma_optimizer.set_target(["k4", "k5", "k6", "s2", "s4"])
# cma_optimizer.set_target(["fov", "k1", "k2", "k3", "k4", "k5", "k6", "p1", "p2", "s1", "s2", "s3", "s4"])
params_optim, error = cma_optimizer.optimize(
    generation = 100, sigma = 0.1, population_size=150, f_scale=error*2)
print("Error:", error)
print("Optimized Parameters:")
for k in params_optim.keys():
    print(f"{k}: {params_optim[k]}")
